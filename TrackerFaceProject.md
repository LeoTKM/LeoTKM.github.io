...
<a href="https://github.com/LeoTKM/FaceTracker" aria-label="GitHub Repository" style="color: #24292e; text-decoration: none; display: inline-block;">
...
The control system is fully integrated into a Raspberry Pi 4B. The program was written in Python and C. Prior to using the Pi, libraries that were prone to errors were tested on my Mac along with an STM32 dev board, where visual diagnostics was available.
...
I'm using a closed-loop PID controller to actively drive the motor, with the target PWM driving frequency adjusted based on the face centre offset/shift provided by the onboard camera via USB. Two magnetic encoders were used to ensure that the motors don't exceed their range of motion. 
...
<div style="margin-top:20px; text-align: center;">
    <figure style="margin: 0;">
        ...
        <figcaption style="margin-top: 8px; color: #666;">Figure 2: PID control diagram</figcaption>
    </figure>
</div>
...
This project integrates multiple ROS2 topics, services and custom msg & srv types as shown in the following diagram. The majority of nodes subscribe to various topics, while the core calculation functions within each node run independently at their own frequencies. This way we ensure that there are sufficient data to process and that there is enough time for the calculations to complete before making any incorrect motion decision. 
